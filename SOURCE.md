...
이 문서는 `covariance-risk-decomposition` 컨텐츠를 만들 때 참고해야 할 로컬 source inventory입니다. 새 코드, 데이터 예시, 문서, 산출물을 만들 때는 이 파일과 `AGENTS.md`를 먼저 확인합니다.

## 프로젝트 식별

- 프로젝트명: 공분산 리스크 분해 (Covariance Risk Decomposition)
- 연구 카테고리: Stochastic Models & Market Risk
- 핵심 질문: 포트폴리오 변동성은 어떤 자산의 개별 변동성과 상관구조에서 주로 발생하는가?

## KAIST-DFMBA 참고 경로

아래 경로는 컨텐츠를 설계할 때 확인할 로컬 참고 위치입니다. 세부 구현은 이 저장소의 목적에 맞게 별도 구조로 정리합니다.

- KAIST-DFMBA/Pension-Fund/
- KAIST-DFMBA/Quant-and-Factor-Investment-Strategies/
- KAIST-DFMBA/Financial-Data-Analysis/

## 참고 초점

- 자산 수익률 패널과 공분산 행렬 구성
- 가중치 벡터를 이용한 포트폴리오 분산 계산
- marginal/component risk contribution 해석
